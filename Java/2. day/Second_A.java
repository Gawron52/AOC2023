...
class Second_A {
    ...
    private static int matching(String data) {
        ...
        while (setMatcher.find()) {
            ...
            if (redMatcher.find()) {
                if (strToNumber(redMatcher.group(1)) > red_max) return 0;
            }
            if (greenMatcher.find()) {
                if (strToNumber(greenMatcher.group(1)) > green_max) return 0;
            }
            if (blueMatcher.find()) {
                if (strToNumber(blueMatcher.group(1)) > blue_max) return 0;
            }
        }
